...
var currentIndex;
var currentCountry = $('#country').val()+ "(Total)";

// last date of the data
var lastDate = new Date();
if(covid_cc_total_timeline.length>0) lastDate = new Date(covid_cc_total_timeline[covid_cc_total_timeline.length - 1].date);
...
var countryIndexMap = {};
var list = {};
if(covid_cc_timeline.length>0)
list = covid_cc_timeline[covid_cc_timeline.length - 1].list;
...
if (index == undefined) {
index = covid_cc_timeline.length - 1;
}

var data = {date:getToday(),list : {}};
if(covid_cc_timeline[index])
data = covid_cc_timeline[index];
// augment with names
...
window.open('../world','_self');
});

homeButton.icon = new am4core.Sprite();
//homeButton.icon.stroke = am4core.color("#ffffff");
homeButton.icon.fill = am4core.color("#ffffff");
homeButton.padding(7, 5, 7, 5);
homeButton.width = 30;
homeButton.tooltipText = "World";
...
mapChart.geodata = am4geodata_cc;
...
title.text = "COVID-19 "+$('#country').val()+" Data";
title.align = "left";
...
var index = Math.round((covid_cc_timeline.length - 1) * slider.start);
...
lineChart.data = JSON.parse(JSON.stringify(covid_cc_total_timeline));
...
// select a country
function selectCountry(mapPolygon,scrolling=true) {
resetHover();
...
var selected = $("tr").filter(function(){return $(this).data("areaid") == mapPolygon.dataItem.id});
//scrollTo
if(scrolling)selected.get(0).scrollIntoView({behavior: 'smooth'});
...
for (var i = 0; i < lineChart.data.length; i++) {
var di = covid_cc_total_timeline[i];
var dataContext = lineChart.data[i];
...
polygon.dataItem.dataContext.recoveredPC = di.recovered ;// population * 100000;
polygon.dataItem.dataContext.active = di.confirmed - di.recovered - di.deaths;
polygon.dataItem.dataContext.activePC = polygon.dataItem.dataContext.active ;
...
for (var i = 0; i < list.length; i++) {
var area = list[i];
var tr = $("<tr>").addClass("area").data("areaid", area.id).data("areaconfirmed", area.confirmed)
.data("areadeath", area.deaths).data("arearecovered", area.recovered).appendTo(table).on("click", function() {
nbclicks+=1;
var id = $(this).data("areaid");
setTimeout(function(){
if(nbclicks==1 || nbclicks==2){
selectCountry(polygonSeries.getPolygonById(id,false));
}nbclicks=0;
},250);
}).hover(function() {
...
nbclicks=2;
$('#cc2').val($(this).data("areaid"));
$('#confirmed').val($(this).data("areaconfirmed"));
$('#death').val($(this).data("areadeath"));
$('#recovered').val($(this).data("arearecovered"));
$('#when').val(getToday());
$('#DTE_Bubble').show('slow');
});
...
var ele = $.grep(am4geodata_cc.features, function (element, index) { return element.id == id;})[0].properties.name;
return ele? ele: id;
}
...
if(str=="1"){
$(".DTE_Form_Success").show("slow");
setTimeout(function(){$(".DTE_Form_Success").hide("slow");},4000);
}
else if(str=="0"){
$(".DTE_Form_Error").show("slow");
setTimeout(function(){$(".DTE_Form_Error").hide("slow");},4000);
clear_data();
}
